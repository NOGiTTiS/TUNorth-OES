...
import { 
  Plus, Calendar, Clock, BookOpen, AlertCircle, 
  Pencil, Trash2, Users, MoreVertical 
} from "lucide-react";
...
export default function ExamListPage() {
  ...
  const getExamStatus = (start: string, end: string) => {
    ...
    if (now > endDate) {
      ...
      return { label: "กำลังดำเนินการ", color: "bg-green-600 hover:bg-green-700" };
    } else {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500">
            สร้างและจัดการชุดข้อสอบสำหรับนักเรียน
          </p>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {loading ? (
           <div className="col-span-full text-center py-10 text-gray-500">กำลังโหลดข้อมูล...</div>
        ) : exams.length === 0 ? (
            ...
            exams.map((exam) => {
                ...
                return (
                  <Card key={exam.ID} className="flex flex-col hover:shadow-md transition-shadow relative group">
                               ...
                               <div className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity">
                                  <DropdownMenu>
                                    ...
                                    <DropdownMenuContent align="end">
                                      ...
                                      <DropdownMenuItem onClick={() => router.push(`/dashboard/exams/create?edit=${exam.ID}`)}> 
                      ...
                      <CardContent className="text-sm space-y-3 text-gray-600 flex-1">
                          ...
                          <div className="flex items-start gap-2 pt-1 border-t">
                              ...
                              <div className="flex flex-wrap gap-1">
                                {(exam as any).target_classes && (exam as any).target_classes.length > 0 ? (
                                    (exam as any).target_classes.slice(0, 3).map((cls: string) => (
                                        <span key={cls} className="bg-slate-100 text-slate-600 px-1.5 rounded text-xs">
                                            {cls}
                                        </span>
                                    ))
                                ) : (
                                    <span className="text-xs text-gray-400">ไม่ระบุห้อง</span>
                                )}
                                {(exam as any).target_classes?.length > 3 && (
                                    <span className="text-xs text-gray-400">+{ (exam as any).target_classes.length - 3 }</span>
                                )}
                              </div>
                          </div>
                      </CardContent>
      ...
      <AlertDialog open={!!deleteId} onOpenChange={(open) => !open && setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              การกระทำนี้ไม่สามารถย้อนกลับได้ หากมีนักเรียนทำข้อสอบชุดนี้ไปแล้ว ข้อมูลการสอบและคะแนนจะถูกลบไปด้วย
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
